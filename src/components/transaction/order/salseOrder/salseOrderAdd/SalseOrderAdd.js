...
const SalseOrderAdd = () => {
    ...
    const getByIdData = async () => {
        try {
            ...
            const formattDueDate = res?.data?.due_date.split("T")[0];
            setFormData({
                date: formattedDate,
                due_date: formattDueDate,
                account: res?.data?.account,
                order_no: res?.data?.order_no,
                transporter: res?.data?.transporter,
                prj_id: res?.data?.prj_id,
                tax_type: res?.data?.tax_type,
                delivery_destination: res?.data?.delivery_destination,
                transport_name: res?.data?.transport_name,
                po_status: res?.data?.po_status,
                        ...
                        <div className="card-body p-0">
                            <div className="table-responsive active-projects style-1">
                                <div className="tbl-caption tbl-caption-2">
                                    <h4 className="heading mb-0 p-2">{params?.id ? "Update" : "Add"} Sales Order</h4>
                                </div>
                                <div className="row">
