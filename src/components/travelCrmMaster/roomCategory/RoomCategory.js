...
import RoomCategoryFilter from "./roomCategoryFilter/RoomCategoryFilter";
import Breadcrumbs from "../../../common/breadcrumb/Breadcrumbs";
import { deletetrcrm, getatrcrm } from "../../../api/login/Login";
...
const RoomCategory = () => {
    ...
    const getTransitionReport = async (input) => {
        ...
        try {
            const res = await getatrcrm(clone)
            if (res?.error == false) {
    ...
    const deleteData = async (id) => {
        try {
            const res = await deletetrcrm(id)
            // console.log(res);
            if (res?.error == false) {
                toastSuccessMessage()
    ...
    return (
        <>
            ...
            <RoomCategoryFilter filterInitial={filterInitial} handleChange={handleChange} getTransitionReport={getTransitionReport} />
            <div>
                            ...
                            <div className="card-body p-0">
                                <div className="table-responsive active-projects style-1">
                                    <div className="tbl-caption">
                                        <h4 className="heading mb-0 p-2">Room Category Master
                                        </h4>
                                        <Link to='/travel-room-category-add' className="btn btn-primary">Add Room Category Master</Link>
                                    </div>
                                                    ...
                                                    return <tr role="row" key={item?._id}>
                                                        ...
                                                        <td>
                                                            <div className="d-flex">
                                                                <Link to={`/travel-room-category-update/${item?._id}`} className="btn btn-primary shadow btn-xs sharp me-1">
